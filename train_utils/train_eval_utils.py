...
def train_one_epoch(model, optimizer, data_loader, data_loader2,device, epoch,
    ...
    loss_test = torch.zeros(1).to(device)
    for i, [images, targets] in enumerate(metric_logger.log_every(data_loader2, print_freq, header)):
        images = list(image.to(device) for image in images)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        ...
        loss_value = losses_reduced.item()
        loss_test = (loss_test * i + loss_value) / (i + 1)
        if i > 100:
            break
    return mloss, now_lr, loss_test
