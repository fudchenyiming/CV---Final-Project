...
def create_model(num_classes,load_pretrain_weights=True):
    ...
    # vgg_feature = vgg(model_name="vgg16", weights_path="./pretrained/vgg16.pth").features
    # backbone = torch.nn.Sequential(*list(vgg_feature._modules.values())[:-1])  # 删除features中最后一个Maxpool层
    # backbone.out_channels = 512
    backbone = resnet50_fpn_backbone(pretrain_path="./pretrained/resnet50.pth",
                                     norm_layer=torch.nn.BatchNorm2d,
                                     trainable_layers=3)
    # https://download.pytorch.org/models/mobilenet_v2-b0353104.pth
    # backbone = MobileNetV2(weights_path="./backbone/mobilenet_v2.pth").features
    # backbone = MobileNetV2(weights_path=None).features


    # backbone.out_channels = 512  # 设置对应backbone输出特征矩阵的channels
    ...
    if load_pretrain_weights:
        ...
        weights_dict = torch.load("./pretrained/fasterrcnn_resnet50_fpn_coco.pth", map_location='cpu')
...
def main():
    ...
    for epoch in range(init_epochs):
        ...
        mean_loss, lr,loss_test = utils.train_one_epoch(model, optimizer, train_data_loader,val_data_loader,
        ...
        writer.add_scalar('test_mAP', coco_info[1], epoch)
        writer.add_scalar('test_loss', loss_test, epoch)

        if epoch in range(init_epochs):
            save_files = {
                'model': model.state_dict(),
                'optimizer': optimizer.state_dict(),
                'lr_scheduler': lr_scheduler.state_dict(),
                'epoch': epoch}
            torch.save(save_files, "./save_weights/imagenet-model-{}.pth".format(epoch))
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        # save weights
        # 仅保存最后5个epoch的权重
        if epoch in range(num_epochs+init_epochs):
            save_files = {
                'model': model.state_dict(),
                'optimizer': optimizer.state_dict(),
                'lr_scheduler': lr_scheduler.state_dict(),
